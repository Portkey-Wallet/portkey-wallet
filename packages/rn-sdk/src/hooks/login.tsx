...
import { request as globalRequest } from 'core/network';
    ...
    async ({
      ...
      try {
        ...
        if (biometricsReady && biometrics === undefined) {
          Loading.hide();
          router.navigate('SetBiometrics', { pin: confirmPin });
...
export function useGoGuardianApproval(isLogin?: boolean) {
  ...
  const requestOrSetPin = useCallback(
    async ({ guardianItem, originChainId, authenticationInfo }: TVerifierAuthParams) => {
      ...
      console.log('dispatch setOriginChainId action exe');
  ...
  const goVerifierDetails = useCallback(
    async ({ guardianItem, originChainId }: TVerifierAuthParams) => {
      ...
      await sleep(200);
      dispatch(setOriginChainId(originChainId));
      router.navigate(PortkeyEntries.VERIFIER_DETAIL_ENTRY, {
        autoLogin: true,
        guardianItem,
  ...
  return useCallback(
    async ({
      ...
      const onConfirm = async () => {
        ...
        Loading.hide();
        dispatch(setOriginChainId(originChainId));
        router.navigate(PortkeyEntries.GUARDIAN_APPROVAL_ENTRY, {
          loginAccount,
          userGuardiansList,
          authenticationInfo,
          initGuardiansStatus,
...
export function useGoSelectVerifier(isLogin?: boolean) {
  ...
  const onConfirmAuth = useCallback(
    async ({ loginAccount, loginType, authenticationInfo, selectedVerifier, chainId }: LoginAuthParams) => {
      ...
      try {
        console.log('useGoSelectVerifier verifyToken');
...
export function useOnLogin(isLogin?: boolean) {
  ...
  const getRegisterInfo = useGetRegisterInfo(request);
  ...
  return useCallback(
    async (params: LoginParams) => {
      ...
      try {
        ...
        console.log('guardianAccounts', guardianAccounts);
        console.log('guardianList', guardianList);
        if (guardianAccounts || guardianList) {
          console.log('goGuardianApproval');
  ...
  return useCallback(
    ({
      ...
      if (walletInfo?.address && pin) {
        ...
        Loading.hide();
        // router.navigate(PortkeyEntries.SET_PIN, {
        //   managerInfo,
        //   guardiansApproved,
        //   verifierInfo,
        //   autoLogin,
        // });
        router.navigate(PortkeyEntries.SET_PIN, {
          deliveredSetPinInfo: convertSetPinData({
            managerInfo,
            guardiansApproved,
            originChainId: originChainId,
          }),
...
// convert data temperately
function convertSetPinData({
  managerInfo,
  guardiansApproved,
  originChainId,
}: {
  ...
  console.log('convertSetPinData originChainId', originChainId);
  const verifiedGuardians = guardiansApproved?.map(item => {
    return {
      type: item.type,
  ...
  const res = {
    normalVerifyPathInfo: {
      fromRecovery: true,
      accountIdentifier: managerInfo.loginAccount,
