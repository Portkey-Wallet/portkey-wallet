...
export const useTestAmountPrice = (
  ...
  _fiat?: FiatType,
  _token?: CryptoItemType,
  initialAmount?: string,
) => {
  // const [amount, setAmount] = useState<string>(initialAmount || '30.73');
  const [amount] = useState<string>(initialAmount || '30.73');
  // const [rate, setRate] = useState<string>('0.31');
  const [rate] = useState<string>('0.31');
  ...
  const refreshReceive = useCallback(async () => {
    ...
    //   setRate(formatAmountShow(rst.cryptoPrice, 2));
    //   setAmount(formatAmountShow(receiveAmount * Number(rst.cryptoPrice), 2));
    // } catch (error) {
  ...
  return { amount, rate, rateRefreshTime };
};
