export enum IMErrorEnum {
  NO_IM_INSTANCE = 'noImInstance',
}

export const CHANNEL_LIST_LIMIT = 20;
export const MESSAGE_LIST_LIMIT = 50;
export const SEARCH_CHANNEL_LIMIT = 50;
export const MAX_INPUT_LENGTH = 500;
export const MAX_FILE_SIZE = 10; // 10M
export const MAX_FILE_SIZE_BYTE = 10 * 1024 * 1024; // 10M
export const RED_PACKAGE_DEFAULT_MEMO = 'Best wishes!';
export const PIN_MESSAGE_LIST_LIMIT = 99;
