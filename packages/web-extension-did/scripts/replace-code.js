...
function applyRule(ruleName, filePath, content) {
  ...
  switch (ruleName) {
    ...
    case 'case81':
      ...
      const case81Pattern = /static\s+async\s+setTGScript\(\)\s*\{\s*if\s*\(\s*typeof\s+document\s*>\s*["']u["']\s*\)\s*return;\s*const\s+\w+\s*=\s*document\.createElement\(["']script["']\);\s*\w+\.src\s*=\s*["']https:\/\/telegram\.org\/js\/telegram-web-app\.js["'],\s*document\.body\.appendChild\(\w+\),\s*\w+\s*\?\s*\.addEventListener\(["']load["'],\s*\({0,2}\s*\(\)\s*=>\s*\{\}\s*\){0,2}\)\s*\}/g;
