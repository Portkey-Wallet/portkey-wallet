import { useCallback, useEffect, useState } from 'react';
// import CommonToast from 'components/CommonToast';
import { MAX_ACCOUNT_NUMBER } from '@portkey-wallet/store/store-eoa/wallet/config';
// import useRouterParams from '@portkey-wallet/hooks/useRouterParams';
import { useLocationState } from 'hooks/router';
import { TWalletInfo } from '@portkey-wallet/types/types-eoa/wallet';
import { useAccountByWallet } from '@portkey-wallet/hooks/hooks-eoa/wallet';
// import navigationService from 'utils/navigationService';
// import { useCheckSecurityLock } from 'hooks/securityLock';
// import { useCredentials } from 'hooks/store';
import singleMessage from 'utils/singleMessage';
import { useNavigate } from 'react-router-dom';
import { SWEventDispatchAccountsChangedWithCurrentAccount } from '../../../../utils/Wallet/account';

type IAccountState = 'idle' | 'adding' | 'added';
export const useEmptyAddress = () => {
  return {
    addAddressDisabled: false,
    // accountAdding: false,
    accountState: 'idle',
    // eslint-disable-next-line @typescript-eslint/no-empty-function
    addNewAddress: () => {},
  };
};

type TRouterParams = {
  action: string;
  routerPin: string;
  pin: string;
  routerWallet: TWalletInfo;
};

export const useAddAddress = ({ walletInfo }: { walletInfo?: TWalletInfo }) => {
  // const [accountAdding, setAccountAdding] = useState(false);
  // const [accountAdded, setAccountAdded] = useState(false);
  const navigate = useNavigate();
  const [accountState, setAccountState] = useState<IAccountState>('idle');
  const { addAccount } = useAccountByWallet(walletInfo);
  const [addAddressDisabled, setAddAddressDisabled] = useState(false);
  // const [failedToastShowed, setFailedToastShowed] = useState(false);
  // const checkSecurityLock = useCheckSecurityLock();
  // const credentials = useCredentials();
  // const { action, routerPin, routerWallet } = useRouterParams<{
  //   action: string;
  //   routerPin: string;
  //   routerWallet: TWalletInfo;
  // }>();
  const { state, search } = useLocationState<TRouterParams>();
  const { action, routerPin, pin, routerWallet } = state || {};
  console.log('useAddAddress: ', action, routerPin, routerWallet, walletInfo);

  useEffect(() => {
    if (!walletInfo) {
      return;
    }
    walletInfo.accountList.length >= MAX_ACCOUNT_NUMBER && setAddAddressDisabled(true);
  }, [walletInfo]);

  useEffect(() => {
    console.log('ADD_ACCOUNT: ', accountState, action);
    // if (action !== 'ADD_ACCOUNT' || accountAdding || accountAdded) {
    if (action !== 'ADD_ACCOUNT' || accountState !== 'idle') {
      return;
    }
    const walletMatched = routerWallet && routerWallet.key === walletInfo?.key;
    if (!walletMatched) {
      return;
    }
    // console.log('ADD_ACCOUNT: ', walletMatched, accountState, action, routerPin, routerWallet);
    // setAccountAdding(true);
    setAccountState('adding');
    setTimeout(() => {
      addAccount(routerPin || pin);
      singleMessage.success('New address added');
      setAccountState('added');
      setTimeout(async () => {
        await SWEventDispatchAccountsChangedWithCurrentAccount();
      }, 100);
      navigate('/wallet/manage' + search, {
        state: {
          action: 'ADDED_ACCOUNT',
        },
      });
      // setAccountAdding(false);
      // setAccountAdded(true);
    }, 100);
  }, [accountState, action, addAccount, routerPin, routerWallet, walletInfo, search]);

  // useEffect(() => {
  //   if (!walletInfo || failedToastShowed) {
  //     return;
  //   }
  //   if (accountAdded && walletInfo.accountList.length >= MAX_ACCOUNT_NUMBER) {
  //     setFailedToastShowed(true);
  //     CommonToast.fail(`Add up to ${MAX_ACCOUNT_NUMBER} addresses per wallet`);
  //   }
  // }, [accountAdded, walletInfo]);

  const addNewAddress = useCallback(async () => {
    if (addAddressDisabled) {
      singleMessage.error(`Add up to ${MAX_ACCOUNT_NUMBER} addresses per wallet`);
      return;
    }
    // singleMessage.success('add address');
    // await checkSecurityLock(() => {
    //   console.log('securePassword: ', credentials);
    //   if (!credentials?.pin) {
    //     // almost impossible
    //     singleMessage.error('Failed to be added');
    //     return;
    //   }
    //   navigationService.pop(1);
    //   navigationService.push('WalletManagement', {
    //     routerWallet: walletInfo,
    //     routerPin: credentials.pin,
    //     action: 'ADD_ACCOUNT',
    //   });
    // }, true);
    navigate('/unlock', {
      state: {
        navigateUrl: '/wallet/manage' + search,
        params: {
          routerWallet: walletInfo,
          // routerPin: pin,
          action: 'ADD_ACCOUNT',
        },
        // routerWallet: walletInfo,
        // routerPin: pin,
        // action: 'ADD_ACCOUNT',
      },
    });
    // const pin = await getPin();
    // console.log('useAddAddress addNewAddress: ', walletInfo, pin);
    // navigate('/wallet/manage', {
    //   state: {
    //     routerWallet: walletInfo,
    //     routerPin: pin,
    //     action: 'ADD_ACCOUNT',
    //   },
    // });
  }, [addAddressDisabled, navigate, walletInfo, search]);

  // const addNewAddress = useCallback(async () => {
  //   if (addAddressDisabled) {
  //     singleMessage.error(`Add up to ${MAX_ACCOUNT_NUMBER} addresses per wallet`);
  //     return;
  //   }
  //   await checkSecurityLock(() => {
  //     console.log('securePassword: ', credentials);
  //     if (!credentials?.pin) {
  //       // almost impossible
  //       singleMessage.error('Failed to be added');
  //       return;
  //     }
  //     navigationService.pop(1);
  //     navigationService.push('WalletManagement', {
  //       routerWallet: walletInfo,
  //       routerPin: credentials.pin,
  //       action: 'ADD_ACCOUNT',
  //     });
  //   }, true);
  // }, [addAddressDisabled, checkSecurityLock, credentials, walletInfo]);

  return {
    // accountAdding,
    // accountAdded,
    accountState,
    // setAccountAdded,
    // setAccountAdding,
    addAddressDisabled,
    addNewAddress,
  };
};
