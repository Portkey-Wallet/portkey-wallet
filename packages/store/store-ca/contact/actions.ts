...
import { FetchContractListAsyncPayloadType, FetchContactListAsyncPayloadTypeV2 } from './type';
import { CONTACT_API_FETCH_SIZE, CONTACT_API_RETRY_LIMIT } from '@portkey-wallet/constants/constants-ca/contact';
import { transContactsToIndexes, transContactsToIndexesV2 } from './utils';
...
export const fetchContactListV2Async = createAsyncThunk<FetchContactListAsyncPayloadTypeV2, boolean | undefined>(
  'contact/fetchContactListV2Async',
  async (isInit = false, thunkAPI) => {
    const {
      contact: contactState,
      wallet: { currentNetwork },
    ...
    if (isInit || contactState.lastModified === 0) {
      ...
      while (page === 1 || contactList.length < totalCount) {
        try {
          console.log('getContactList', page, errorTimes);
          const response = await getContactListV2(baseUrl, {
            page,
            size: CONTACT_API_FETCH_SIZE,
            modificationTime: new Date(modificationTime).toISOString(),
          });
      ...
      return {
        isInit: true,
        contactIndexList: transContactsToIndexesV2(contactList),
        lastModified: modificationTime,
      };
    }

    // update getContactEventList
    let eventList: IContactItemType[] = [];
    let page = 1,
      errorTimes = 0,
      totalCount = 0;
