...
export type TopMenuQueryVariables = Types.Exact<{
  ...
  filter6?: Types.InputMaybe<Types.TopMenu_Filter>;
...
export const TopMenuDocument = gql`
  query topMenu(
    ...
    topMenu(filter: $filter6, sort: $sort6, limit: $limit6, offset: $offset6, page: $page6, search: $search6) {
      date_created
      date_created_func {
        year
        month
 ...
 * __useTopMenuQuery__
 *
 * To run a query within a React component, call `useTopMenuQuery` and pass it any options that fit your needs.
 * When your component renders, `useTopMenuQuery` returns an object from Apollo Client that contains loading, error, and data properties
 * you can use to render your UI.
 *
 * @param baseOptions options that will be passed into the query, supported options are listed on: https://www.apollographql.com/docs/react/api/react-hooks/#options;
 *
 * @example
 * const { data, loading, error } = useTopMenuQuery({
 *   variables: {
 ...
 * });
 */
export function useTopMenuQuery(baseOptions?: Apollo.QueryHookOptions<TopMenuQuery, TopMenuQueryVariables>) {
  const options = { ...defaultOptions, ...baseOptions };
  return Apollo.useQuery<TopMenuQuery, TopMenuQueryVariables>(TopMenuDocument, options);
}
export function useTopMenuLazyQuery(baseOptions?: Apollo.LazyQueryHookOptions<TopMenuQuery, TopMenuQueryVariables>) {
  const options = { ...defaultOptions, ...baseOptions };
  return Apollo.useLazyQuery<TopMenuQuery, TopMenuQueryVariables>(TopMenuDocument, options);
}
export type TopMenuQueryHookResult = ReturnType<typeof useTopMenuQuery>;
export type TopMenuLazyQueryHookResult = ReturnType<typeof useTopMenuLazyQuery>;
export type TopMenuQueryResult = Apollo.QueryResult<TopMenuQuery, TopMenuQueryVariables>;
